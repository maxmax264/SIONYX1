...
public partial class App : Application
{
    ...
    protected override async void OnStartup(StartupEventArgs e)
    {
        ...
        if (!isNew)
        {
            MessageBox.Show("SIONYX כבר פועל במחשב זה.\n\nלא ניתן להפעיל יותר מעותק אחד של התוכנה במקביל.\n\nאנא סגור את החלון הקיים ונסה שוב.", "SIONYX", MessageBoxButton.OK, MessageBoxImage.Information);
            Shutdown();
            return;
        }
    ...
    private void ShowAuthWindow()
    {
        ...
        _systemServices.ForceLogoutReceived += async () =>
        {
            AlertDialog.Show(
                "התנתקות מרחוק",
                "המנהל התנתק אותך מהמערכת מרחוק.\n\nאנא התחבר מחדש כדי להמשיך להשתמש במחשב.",
        ...
        try
        {
            ...
            if (dialog.ShowDialog() == true)
            {
                        ...
                        Current.Dispatcher.Invoke(() =>
                        {
                            ...
                            _trayIcon.CheckUpdateRequested += () =>
                            {
                                _ = Task.Run(async () =>
                                {
                                    ...
                                    var msg = hasUpdate ? $"נמצאה גרסה חדשה: {latestVersion}" : "אין עדכון זמין - המערכת מעודכנת";
                                    Current.Dispatcher.Invoke(() => _trayIcon?.ShowBalloon("בדיקת עדכונים - SIONYX", msg));
                                });
                            };
                ...
                else
                {
                    ...
                    MessageBox.Show("הסיסמה שגויה. אנא נסה שוב.", "SIONYX", MessageBoxButton.OK, MessageBoxImage.Warning);
                }
            }
        }
        catch (Exception ex)
