...
public class PrintMonitorService : BaseService, IDisposable
{
    ...
    private void OnPrintBalanceUpdated(string eventType, JsonElement? data)
    {
        ...
        try
        {
            ...
            Logger.Information("[PRINT] Live printBalance update from Firebase: {Old} -> {New} (delta={Delta})", _cachedBudget, newBalance, newBalance - (_cachedBudget ?? 0));
    ...
    private void InitializeKnownJobs()
    {
        ...
        Logger.Information("[PRINT] Found {Count} printer(s) on startup", printers.Count);
        ...
        try
        {
            ...
            foreach (var printer in printers)
            {
                ...
                foreach (var jobId in currentIds)
                {
                    ...
                    if (shouldProcess)
                    {
                        Logger.Information("New print job detected: ID={JobId} on '{Printer}' - processing", jobId, printer);
