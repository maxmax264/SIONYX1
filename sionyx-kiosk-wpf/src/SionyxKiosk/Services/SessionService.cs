...
public class SessionService : BaseService, ISessionService
{
    ...
    public async Task<ServiceResult> StartSessionAsync(int initialRemainingTime)
    {
        ...
        // Stop idle listener and start session listener
        _idleRemainingTimeListener?.Stop();
    ...
    private void OnRemainingTimeUpdated(string eventType, JsonElement? data)
    {
        ...
        try
        {
            if (_isFirstRemainingTimeEvent && !_isIdleListener) { _isFirstRemainingTimeEvent = false; return; }
        _isFirstRemainingTimeEvent = false;
            ...
            Logger.Information("[SESSION] Live remainingTime update: {Old}s -> {New}s (delta={Delta}s)", RemainingTime, newTime, newTime - RemainingTime);
